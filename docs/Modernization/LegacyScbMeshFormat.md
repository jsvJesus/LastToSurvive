...
Names are treated as bounded legacy byte strings without embedded NUL rather than claimed as UTF-8. Material names are deduplicated by exact byte value in first-appearance order. Material chunks are canonical: the first starts at index zero, every later start equals the previous end, every range contains whole triangles, and the final end equals `indexCount`.

Normals are safely normalized. Tangents must be finite and non-zero, are Gram-Schmidt orthogonalized against the normalized normal, and are then normalized. A tangent that becomes degenerate is rejected rather than silently replaced. Tangent sign values greater than zero map to `+1`, and zero/negative values map to `-1`, preserving legacy handedness deterministically.
