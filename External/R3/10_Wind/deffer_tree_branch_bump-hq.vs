 ...
 * @ Modified time: 2025-04-05 15:36:32
...
v2p_bumped 	main 	(v_tree I)
{
...
#else
	float3 wind_result	= ssfx_wind_tree_branches(pos, H, tc.y, ssfx_wind_setup(), false);

	// Prev Position
	P_prev = float4(pos.xyz + ssfx_wind_tree_branches(pos, H, tc.y, ssfx_wind_setup(), true), 1);
#endif
	...
	for (int p = 0; p < SSFX_INT_GRASS + 1; p++)
	{
		...
		P_prev.xz += bend_dir.xz * 2.25f * rstr.yy * height_limit; 			// Horizontal
		P_prev.y -= bend * 0.67f * rstr.z * height_limit * dir_limit.y;		// Vertical
	}
#endif
