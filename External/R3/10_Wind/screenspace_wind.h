 ...
 * @ Modified time: 2025-02-04 02:27
...
uniform float4 ssfx_wind_anim_prev;
...
float3 ssfx_wind_grass(float3 pos, float H, wind_setup W, bool prev)
{
	float4 anim = prev ? ssfx_wind_anim_prev : ssfx_wind_anim;

	// Height Limit. ( Add stiffness to tall grass )
	float HLimit = saturate(H * H - 0.01f) * saturate(1.0f - H * 0.1f);
	
	// Offset animation
	float2 Offset = -anim.xy * W.grass_animspeed;
...
float3 ssfx_wind_tree_trunk(float3 pos, float Tree_H, wind_setup W, bool prev)
{
	float4 anim = prev ? ssfx_wind_anim_prev : ssfx_wind_anim;

	// Phase ( from matrix ) + Offset 
	float Phase = m_xform._24 + anim.z * W.trees_trunk_animspeed;
...
float3 ssfx_wind_tree_branches(float3 pos, float Tree_H, float tc_y, wind_setup W, bool prev)
{
	float4 anim = prev ? ssfx_wind_anim_prev : ssfx_wind_anim;

	// UV Offset
	float2 Offset = -anim.xy * W.trees_animspeed;
	...
	// Trunk position
	float3 Trunk = ssfx_wind_tree_trunk(pos, Tree_H, W, prev);
