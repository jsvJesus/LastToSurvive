...
float3 ssfx_lut_pp(float3 base_col)
{
	...
	// Sample LUTs
	float3 lut_col = lerp(	s_lut_atlas.SampleLevel(smp_linear, uvs.xy + grp_offset, 0).rgb, 
							s_lut_atlas.SampleLevel(smp_linear, uvs.zw + grp_offset, 0).rgb,
							lut_frac);
	...
	float3 SecondLUT = lerp(s_lut_atlas.SampleLevel(smp_linear, uvs.xy + grp_offset, 0).rgb,
							s_lut_atlas.SampleLevel(smp_linear, uvs.zw + grp_offset, 0).rgb,
							lut_frac);
