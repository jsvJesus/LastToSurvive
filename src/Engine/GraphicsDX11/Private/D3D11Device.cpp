...
namespace engine::graphics::d3d11
{
    ...
    namespace
    {
            ...
            bool& outDebugEnabled) noexcept
        {
            ...
            auto createForDriver = [&](const D3D_DRIVER_TYPE driver,
                                       const UINT createFlags) noexcept
            {
                HRESULT result = D3D11CreateDevice(
                    nullptr, driver, nullptr, createFlags,
                    featureLevels.data(),
            ...
            HRESULT result = createForDriver(D3D_DRIVER_TYPE_HARDWARE, flags);
            if (FAILED(result) && requestDebug)
            {
                flags &= ~D3D11_CREATE_DEVICE_DEBUG;
                result = createForDriver(D3D_DRIVER_TYPE_HARDWARE, flags);
            }
            if (FAILED(result))
            {
                result = createForDriver(D3D_DRIVER_TYPE_WARP, flags);
            }

            outDebugEnabled = SUCCEEDED(result) &&
            ...
            bool& outTearingSupported) noexcept
        {
            ...
            if (SUCCEEDED(result))
            {
                ...
                result = factory5.Get()->CheckFeatureSupport(
                    ...
                    sizeof(allowTearing));
                outTearingSupported =
                    SUCCEEDED(result) && allowTearing == TRUE;
            }
        ...
        const RenderDeviceDesc& desc) noexcept
    {
        ...
        HRESULT result = CreateNativeDevice(
        ...
        GraphicsResult factoryResult = AcquireFactory(
            impl_->device.Get(), impl_->factory,
        ...
        impl_->contextView.Attach(impl_->immediateContext.Get());
        ...
        std::unique_ptr<SwapChain>& outSwapChain) noexcept
    {
        ...
        return D3D11SwapChain::Create(
            impl_->device.Get(), impl_->factory.Get(),
            impl_->tearingSupported, desc, outSwapChain);
    }
        ...
        TextureHandle& outTexture) noexcept
    {
        ...
        const GraphicsResult result = impl_->resources.CreateTexture(
            impl_->device.Get(), desc, initialData,
            initialDataCount, outTexture);
        if (result == GraphicsResult::DeviceLost ||
    ...
    {
        if (!impl_ || (impl_->state != DeviceState::Ready &&
                       impl_->state != DeviceState::Removed))
        {
            return GraphicsResult::InvalidState;
        }
        ...
        BufferHandle& outBuffer) noexcept
    {
        ...
        const GraphicsResult result = impl_->resources.CreateBuffer(
            impl_->device.Get(), desc, initialData, outBuffer);
        if (result == GraphicsResult::DeviceLost ||
            result == GraphicsResult::DeviceRemoved)
        {
            (void)CheckDeviceStatus();
    ...
    {
        return impl_ ? static_cast<std::uint32_t>(impl_->featureLevel) : 0U;
    }
    ...
    GraphicsResult D3D11Device::CheckDeviceStatus() noexcept
    {
        ...
        const HRESULT reason = impl_->device.Get()->GetDeviceRemovedReason();
        if (reason == S_OK)
            return GraphicsResult::Success;
        ...
        const TextureHandle texture) const noexcept
    {
        ...
        const auto* resource = impl_->resources.GetTexture(texture);
    ...
    {
        if (!impl_) return nullptr;
        const auto* resource = impl_->resources.GetBuffer(buffer);
        return resource ? resource->native.Get() : nullptr;
    }
