...
int main()
{
    ...
    const fs::path secondRoot=base/L"Different Absolute Checkout";const fs::path secondShader=secondRoot/L"test shader.hlsl";
    if(!Check(Write(secondRoot/L"common.hlsli","float4 Color(){return float4(1,0,0,1);}\n")&&Write(secondShader,"#include \"common.hlsli\"\nfloat4 VSMain(float4 p:POSITION):SV_Position{return p;}\nfloat4 PSMain():SV_Target{return Color();}\n"),"second shader checkout fixtures"))return 1;
    auto secondOptions=shaderOptions;secondOptions.input=secondShader;secondOptions.includeRoot=secondRoot;engine::assets::AssetData shaderFromSecondRoot;
    if(!Check(engine::assets::Succeeded(lts::asset_cooker::CookShader(secondOptions,shaderFromSecondRoot,shaderDiagnostics))&&shaderA.GetSize()==shaderFromSecondRoot.GetSize()&&std::memcmp(shaderA.GetData(),shaderFromSecondRoot.GetData(),shaderA.GetSize())==0,"shader output independent of absolute checkout path"))return 1;
    if(!Check(Write(secondRoot/L"common.hlsli","float4 Color(){return float4(0,1,0,1);}\n")&&engine::assets::Succeeded(lts::asset_cooker::CookShader(secondOptions,shaderB,shaderDiagnostics))&&(shaderA.GetSize()!=shaderB.GetSize()||std::memcmp(shaderA.GetData(),shaderB.GetData(),shaderA.GetSize())!=0),"include bytes change shader output"))return 1;
