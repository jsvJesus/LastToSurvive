...
namespace lts::editor
{
    ...
    bool EditorLevelDocument::Initialize(
        const engine::platform::
            NativeWindowHandle mainWindow,
        const EditorSceneDocument&
    ...
    void EditorLevelDocument::Shutdown() noexcept
    {
    ...
    void EditorLevelDocument::QueueCommand(
    ...
    bool EditorLevelDocument::WasPressed(
        const int virtualKey) noexcept
    {
        if (
            virtualKey < 0 ||
    ...
    void EditorLevelDocument::PollShortcuts() noexcept
    {
    ...
    EditorLevelDocument::OperationResult
        EditorLevelDocument::
            ConfirmSaveChanges(
                EditorSceneDocument&
                    sceneDocument) noexcept
    ...
    EditorLevelDocument::OperationResult
        EditorLevelDocument::CreateNewLevel(
            EditorSceneDocument& sceneDocument,
            EditorCommandHistory&
                commandHistory) noexcept
    {
    ...
    EditorLevelDocument::OperationResult
        EditorLevelDocument::SaveLevelAs(
            EditorSceneDocument&
                sceneDocument) noexcept
    {
        std::filesystem::path path;
    ...
    bool EditorLevelDocument::ShowFileDialog(
    ...
    void EditorLevelDocument::ShowError(
    ...
    LRESULT CALLBACK
        EditorLevelDocument::WindowProcedure(
            const HWND window,
            const UINT message,
            const WPARAM wParam,
    ...
    {
        auto* const self =
            static_cast<EditorLevelDocument*>(
                GetPropW(
                    window,
                    LevelDocumentPropertyName));
