...
{
namespace { struct Vertex{DirectX::XMFLOAT3 position;DirectX::XMFLOAT3 normal;DirectX::XMFLOAT2 uv;}; struct alignas(16) Constants{DirectX::XMFLOAT4X4 world;DirectX::XMFLOAT4X4 viewProjection;std::array<DirectX::XMFLOAT4,18> placement;std::array<DirectX::XMFLOAT4,18> layerParameters;};
...
public:
    bool Initialize(engine::graphics::RenderDevice& d) noexcept
    {
        ...
        {
            std::uint32_t decodedWidth=0U,decodedHeight=0U;
            const bool decoded=i<terrain.masks.size()&&DecodeEmbeddedMask(
                terrain,terrain.masks[i],maskPixels_[i],decodedWidth,decodedHeight);
            if(decoded)
            {
                maskWidth_=decodedWidth;maskHeight_=decodedHeight;
            }
            else
            {
                maskPixels_[i].assign(
                    static_cast<std::size_t>(maskWidth_)*maskHeight_*4U,std::byte{0});
            }
        }
    ...
    engine::graphics::GraphicsResult Render(engine::graphics::CommandContext& c,const EditorSceneDocument& document,const DirectX::XMFLOAT4X4& vp,const DirectX::XMFLOAT3& camera) noexcept
    {
        ...
        Constants constants{};
        const auto worldMatrix=DirectX::XMMatrixScaling(actor->transform.scale[0],actor->transform.scale[1],actor->transform.scale[2])*DirectX::XMMatrixRotationRollPitchYaw(DirectX::XMConvertToRadians(actor->transform.rotationDegrees[0]),DirectX::XMConvertToRadians(actor->transform.rotationDegrees[1]),DirectX::XMConvertToRadians(actor->transform.rotationDegrees[2]))*DirectX::XMMatrixTranslation(actor->transform.position[0],actor->transform.position[1],actor->transform.position[2]);
        DirectX::XMStoreFloat4x4(&constants.world,worldMatrix);constants.viewProjection=vp;for(std::size_t i=0;i<constants.placement.size();++i){constants.placement[i]={512.0F,512.0F,0.0F,0.0F};constants.layerParameters[i]={1.0F,0.0F,0.0F,0.0F};if(i<actor->terrain->layers.size()){const auto& layer=actor->terrain->layers[i];constants.placement[i]={layer.scaleU,layer.scaleV,layer.offsetU,layer.offsetV};constants.layerParameters[i].x=layer.visible?1.0F:0.0F;}}
        auto r=c.UpdateBuffer(constants_,&constants,sizeof(constants));if(engine::graphics::Failed(r))return r;r=c.SetGraphicsPipeline(pipeline_);if(engine::graphics::Failed(r))return r;engine::graphics::VertexBufferBinding vb{vertex_,sizeof(Vertex),0};engine::graphics::IndexBufferBinding ib{index_,0};r=c.SetVertexBuffers(0,&vb,1);if(!engine::graphics::Failed(r))r=c.SetIndexBuffer(ib);if(!engine::graphics::Failed(r))r=c.SetConstantBuffers(engine::graphics::ShaderStage::Vertex,0,&constants_,1);if(!engine::graphics::Failed(r))r=c.SetConstantBuffers(engine::graphics::ShaderStage::Pixel,0,&constants_,1);if(!engine::graphics::Failed(r))r=c.SetShaderResources(engine::graphics::ShaderStage::Pixel,0,masks_.data(),masks_.size());if(!engine::graphics::Failed(r))r=c.SetShaderResources(engine::graphics::ShaderStage::Pixel,6,materials_.data(),materials_.size());if(!engine::graphics::Failed(r))r=c.SetSamplers(engine::graphics::ShaderStage::Pixel,0,&sampler_,1);
