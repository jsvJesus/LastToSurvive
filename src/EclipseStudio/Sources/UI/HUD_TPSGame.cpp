...
bool CheckCameraCollision(r3dPoint3D& camPos, const r3dPoint3D& target, bool checkCamera)
{
	...
	if(motionLen > 0.1f)
	{
		...
		PxSweepBufferN<32> sweepResults;
		while(g_pPhysicsWorld->PhysXScene->sweep(camSphere, camPose, PxVec3(motion.x, motion.y, motion.z), motionLen, sweepResults, PxSceneQueryFlag::eINITIAL_OVERLAP|PxSceneQueryFlag::eNORMAL, filter) && LoopBreaker<MaxLoopBreaker)
		{
			/* PxVec3 collNormal = PxVec3(0,0,0);
			for(int i=0; i<sweepResults.getNbAnyHits(); ++i)
			{
			collNormal += sweepResults.getAnyHit(i).normal;
			}
...
void ProcessPlayerMovement(obj_Player* pl, bool editor_debug )
{
	...
	if(pl->bProne)
	{
		...
		PxSceneQueryFilterData filter(PxFilterData(COLLIDABLE_STATIC_MASK, 0, 0, 0), PxSceneQueryFilterFlag::eSTATIC);
		PxOverlapBuffer overlapHit;
		force_prone = g_pPhysicsWorld->PhysXScene->overlap(bbox, pose, overlapHit, filter);
	}
