...
DWORD GraphSettingsToVars(const GraphicSettings& settings)
{
	...
	r_decals_proximity_multiplier->SetFloat(
		1.0f - float(settings.decoration_quality) / (S_ULTRA + 1)
	);
...
void FillDefaultSettings(GraphicSettings& settings, r3dDevStrength strength)
{
	...
	default:
		r3dError("FillDefaultSettings: unknown strength");
		break;
	}
}
...
{
	const float halfFOV = R3D_DEG2RAD(halfFovInDegrees);
	const float halfH = boxSize.y * 0.5f;
	const float treeR = R3D_MAX(boxSize.x, boxSize.z);

	const float t = tanf(halfFOV);
	const float distance = (halfH / t) + treeR;

	return distance;
}

void getWeaponParamForUI(
	const WeaponConfig* wc,
	int* damage,
	int* spread,
	int* firerate,
	int* recoil
...
)
{
	...
	if (mindamage) *mindamage = d1;
	if (maxdamage) *maxdamage = d2;
	if (minfirerate) *minfirerate = fr1;
	if (maxfirerate) *maxfirerate = fr2;
	if (minclip) *minclip = c1;
	if (maxclip) *maxclip = c2;
	if (minrange) *minrange = r1;
	if (maxrange) *maxrange = r2;
}
...
void getAdditionalDescForItem(uint32_t itemID, int Var1, int Var2, char* res)
{
	...
	if (
		wc &&
		wc->category != storecat_UsableItem &&
		wc->category != storecat_GRENADE &&
		wc->category != storecat_MELEE
	)
	{
		const int ClipId = Var2 < 0 ? wc->FPSDefaultID[WPN_ATTM_CLIP] : Var2;
		const WeaponAttachmentConfig* clip =
			g_pWeaponArmory->getAttachmentConfig(ClipId);

		if (clip)
		{
			const int ammoLeft = Var1 < 0 ? clip->m_Clipsize : Var1;

			if (ammoLeft > 0)
			{
				sprintf(
					res,
					ammoLeft == 1 ? "%s: %d bullet left" : "%s: %d bullets left",
					clip->m_StoreName,
					ammoLeft
				);
			}
	...
	{
		const int ammoLeft = Var1 < 0 ? wac->m_Clipsize : Var1;

		if (ammoLeft > 0)
		{
			sprintf(
				res,
				ammoLeft == 1 ? "%d bullet left" : "%d bullets left",
				ammoLeft
			);
		}
		else
