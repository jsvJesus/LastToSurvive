...
BOOL obj_Bullet::Update()
{
	...
	if(g_pPhysicsWorld->PhysXScene->sweep(sphere, pose, PxVec3(motion.x, motion.y, motion.z), motionLen, sweepHit, PxSceneQueryFlag::eINITIAL_OVERLAP|PxSceneQueryFlag::eIMPACT|PxSceneQueryFlag::eNORMAL, filter))
	{
		hit = sweepHit.block;
...
bool obj_Bullet::OnHit( PxSweepHit &hit )
{
	...
	if( hit.actor && (target = static_cast<PhysicsCallbackObject*>(hit.actor->userData)))
	{

		hitActorName = hit.actor->getName(); 
