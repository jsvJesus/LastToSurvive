...
static PxShape* R3DCreateTerrainShape(PxRigidActor* actor, const PxGeometry& geometry, PxMaterial& material)
{
...
r3dTerrain2::UpdateHFShape()
{
	...
	r3d_assert( shapes[0]->getGeometry().getType() == PxGeometryType::eHEIGHTFIELD );
...
r3dTerrain2::GetHFShape() const
{
	...
	m_PhysicsTerrain->getShapes(&shapes[0], 1);
	r3d_assert( shapes[0]->getGeometry().getType() == PxGeometryType::eHEIGHTFIELD );
	PxGeometryHolder geometry(shapes[0]->getGeometry());
	PxHeightFieldGeometry geom = geometry.heightField();
...
r3dTerrain2::UpdatePhysHeightField( const Shorts& source )
{
	...
	m_PhysicsHeightField = PxCreateHeightField( m_PhysicsHeightFieldDesc , g_pPhysicsWorld->PhysXSDK->getPhysicsInsertionCallback());
	...
	PxShape* aHeightFieldShape = R3DCreateTerrainShape(m_PhysicsTerrain, shapeGeom, *g_pPhysicsWorld->defaultMaterial);
