...
#pragma comment(lib, "libgfx_air.lib")
...
// 	loader.GetFontCacheManager()->SetMaxRasterScale(1.2f);
	...
	// Create renderer.
	RendererHAL = new Scaleform::Render::D3D9::HAL(NULL);
...
void r3dScaleformMovie::UpdateAndDraw(bool skipDraw, bool skipAdvance)
{
	...
	if(!skipAdvance)
	{
		...
		float frameDelta = 1.0f / R3D_MAX(pMovie->GetFrameRate(), 1.0f);
		if(timeForNextUpdate <= curTime)
		{
			float delta = curTime-timePrevUpdate;
			if(delta <= 0.0f || delta > frameDelta)
				delta = frameDelta;
			timePrevUpdate = curTime;
			pMovie->Advance(delta);
			timeForNextUpdate = curTime + frameDelta;
