...
static PxShape* R3DCreateShape(PxRigidActor* actor, const PxGeometry& geometry, PxMaterial& material)
{
	PxShape* shape = g_pPhysicsWorld->PhysXSDK->createShape(geometry, material);
	r3d_assert(shape);
...
static bool CookTrianglePhysicsMesh(const PhysicsObjectConfig& params, const r3dMesh* objectMesh)
{
#ifdef WO_SERVER
	return false;
#else
	...
	{
		char collisionMeshFilename[256];
		r3dscpy(collisionMeshFilename, params.meshFilename);
		...
		bool cooked = g_pPhysicsWorld->CookMesh(collisionMesh, params.meshFilename);
		SAFE_DELETE(collisionMesh);
		return cooked;
	}

	r3d_assert(objectMesh);
	return g_pPhysicsWorld->CookMesh(objectMesh);
#endif
}

static bool CookConvexPhysicsMesh(const PhysicsObjectConfig& params, const r3dMesh* objectMesh)
{
#ifdef WO_SERVER
	return false;
#else
	if(params.needExplicitCollisionMesh)
	{
		...
		r3dMesh* collisionMesh = r3dGOBAddMesh(collisionMeshFilename, false);
		bool cooked = g_pPhysicsWorld->CookConvexMesh(collisionMesh, params.meshFilename);
		SAFE_DELETE(collisionMesh);
		return cooked;
	}

	r3d_assert(objectMesh);
	return g_pPhysicsWorld->CookConvexMesh(objectMesh);
#endif
}

static PxTriangleMesh* LoadTrianglePhysicsMeshOrRecook(const char* cookedMeshFilename, const PhysicsObjectConfig& params, const r3dMesh* objectMesh)
{
	PxTriangleMesh* mesh = g_pPhysicsWorld->getCookedMesh(cookedMeshFilename);
	if(mesh)
		return mesh;

	r3dOutToLog("PhysX: stale or unreadable cooked mesh '%s', recooking\n", cookedMeshFilename);
	if(!CookTrianglePhysicsMesh(params, objectMesh))
		return NULL;

	return g_pPhysicsWorld->getCookedMesh(cookedMeshFilename);
}

static PxConvexMesh* LoadConvexPhysicsMeshOrRecook(const char* cookedMeshFilename, const PhysicsObjectConfig& params, const r3dMesh* objectMesh)
{
	PxConvexMesh* mesh = g_pPhysicsWorld->getConvexMesh(cookedMeshFilename);
	if(mesh)
		return mesh;

	r3dOutToLog("PhysX: stale or unreadable cooked convex mesh '%s', recooking\n", cookedMeshFilename);
	if(!CookConvexPhysicsMesh(params, objectMesh))
		return NULL;

	return g_pPhysicsWorld->getConvexMesh(cookedMeshFilename);
}

static PxTriangleMesh* LoadPlayerOnlyTriangleMeshOrRecook(const char* cookedMeshFilename, const char* rawMeshFilename)
{
	PxTriangleMesh* mesh = g_pPhysicsWorld->getCookedMesh(cookedMeshFilename);
	if(mesh)
		return mesh;
	...
	return NULL;
#else
	r3dOutToLog("PhysX: stale or unreadable player-only cooked mesh '%s', recooking\n", cookedMeshFilename);
	r3dMesh* playerOnlyMesh = r3dGOBAddMesh(rawMeshFilename, false);
	bool cooked = g_pPhysicsWorld->CookMesh(playerOnlyMesh);
...
static PxConvexMesh* LoadPlayerOnlyConvexMeshOrRecook(const char* cookedMeshFilename, const char* rawMeshFilename)
{
	PxConvexMesh* mesh = g_pPhysicsWorld->getConvexMesh(cookedMeshFilename);
	if(mesh)
		return mesh;
	...
	return NULL;
#else
	r3dOutToLog("PhysX: stale or unreadable player-only cooked convex mesh '%s', recooking\n", cookedMeshFilename);
	r3dMesh* playerOnlyMesh = r3dGOBAddMesh(rawMeshFilename, false);
	bool cooked = g_pPhysicsWorld->CookConvexMesh(playerOnlyMesh);
	SAFE_DELETE(playerOnlyMesh);
	if(!cooked)
		return NULL;
...
// 	}
	...
	{
		shape = R3DCreateShape(actor, PxBoxGeometry(objectSize.x*0.5f, objectSize.y*0.5f, objectSize.z*0.5f), params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
	}
	else if(params.type == PHYSICS_TYPE_SPHERE)
	{
		shape = R3DCreateShape(actor, PxSphereGeometry(R3D_MAX(R3D_MAX(objectSize.x*0.5f, objectSize.y*0.5f), objectSize.z*0.5f)), params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
	}
	else if(params.type == PHYSICS_TYPE_MESH)
	{
		...
		PxTriangleMesh* triangleMesh = LoadTrianglePhysicsMeshOrRecook(cookedMeshFilename, params, objectMesh);
		if(!triangleMesh)
			return NULL;

		PxTriangleMeshGeometry geom(triangleMesh);
		shape = R3DCreateShape(actor, geom, params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
		...
		if(r3dFileExists(cookedMeshFilename))
		{
			...
			{
				PxTriangleMeshGeometry geom_player(playerOnlyTriangleMesh);
				PxShape* playerShape = R3DCreateShape(actor, geom_player, params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
	...
	else if(params.type == PHYSICS_TYPE_CONVEX)
	{
		...
		PxConvexMesh* convexMesh = LoadConvexPhysicsMeshOrRecook(cookedMeshFilename, params, objectMesh);
		if(!convexMesh)
			return NULL;
		...
		if(r3dFileExists(cookedMeshFilename))
		{
			...
			{
				PxConvexMeshGeometry geom_player(playerOnlyConvexMesh);
				PxShape* playerShape = R3DCreateShape(actor, geom_player, params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
				PxFilterData filterData(PHYSCOLL_PLAYER_ONLY_GEOMETRY, 0, 0, 0); // new geometry is player only
				playerShape->setSimulationFilterData(filterData);
				PxFilterData qfilterData(1<<PHYSCOLL_PLAYER_ONLY_GEOMETRY, 0, 0, 0); // new geometry is player only
	...
	{
		R3DCreateShape(actor, PxBoxGeometry(objectSize.x*0.5f, objectSize.y*0.5f, objectSize.z*0.5f), params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
	}
...
//	r3dOutToLog("Creating static phys obj at: %.2f, %.2f, %.2f\n", objectPos.x, objectPos.y, objectPos.z);
	...
	else if(params.type == PHYSICS_TYPE_MESH || (params.type == PHYSICS_TYPE_CONVEX && !params.isTrigger))
	{
		...
		PxTriangleMeshGeometry geom(triangleMesh, optionalScale);
		shape = R3DCreateShape(actor, geom, params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
	...
	else if(params.type == PHYSICS_TYPE_CONVEX)
	{
		...
		PxConvexMeshGeometry geom(convexMesh, optionalScale);
		shape = R3DCreateShape(actor, geom, params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
	...
	if(params.needBoxCollision)
	{
		PxShape* boxCollisionShape = R3DCreateShape(actor, PxBoxGeometry(objectSize.x*0.5f, objectSize.y*0.5f, objectSize.z*0.5f), params.requireNoBounceMaterial?*(g_pPhysicsWorld->noBounceMaterial):*(g_pPhysicsWorld->defaultMaterial));
...
void ControllerPhysObj::Move(const r3dPoint3D& move, float sharpness)
{
	...
	for(int cur_step=0; cur_step<steps; cur_step++) 
	{
		...
		if(!g_pPhysicsWorld->PhysXScene->overlap(bbox, pose, overlapHit, filter)) 
		{
			continue;
		}
		...
		if(dd.magnitudeSquared() > 0)
		{
			...
			PxFilterData filterData(COLLIDABLE_PLAYER_COLLIDABLE_MASK, 0, 0, 0);
			PxControllerFilters cf;
			cf.mFilterData = &filterData;
	...
	// check if player fell through world and don't allow him to do this
	{
		...
		PxRaycastBuffer hit;
		if(!g_pPhysicsWorld->PhysXScene->raycast(PxVec3(new_pos.x, new_pos.y, new_pos.z), PxVec3(0,-1,0), 1000.0f, hit, PxSceneQueryFlag::eDISTANCE, filter)) // nothing below our feet?
		{
