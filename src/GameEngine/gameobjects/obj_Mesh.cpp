...
r3dMesh *r3dGOBAddMesh(const char* fname, bool addToLibrary, bool use_default_material, bool allow_async, bool player_mesh )
{
	if(!fname || !*fname)
	{
		r3dOutToLog("r3dGOBAddMesh: empty mesh filename\n");
		return 0;
	}
	...
	if(!szFixedName[0])
	{
		r3dOutToLog("r3dGOBAddMesh: failed to normalize mesh filename '%s'\n", fname);
		return 0;
	}
	...
	for (size_t i=0;i<s_MeshCache.size();++i)
	{
		...
		if(!mesh)
		{
			r3dOutToLog("r3dGOBAddMesh: null mesh cache entry at %u\n", (unsigned)i);
			continue;
		}

		const char* cachedName = mesh->FileName.c_str();
		if(!cachedName || !*cachedName)
		{
			r3dOutToLog("r3dGOBAddMesh: mesh cache entry %u has empty filename (%p)\n", (unsigned)i, mesh);
			continue;
		}

		if( r3dIsSamePath(cachedName, szFixedName) ) 
		{
...
BOOL MeshGameObject::Load(const char* fname)
{
	if(!fname || !*fname)
	{
		r3dOutToLog("MeshGameObject::Load: empty mesh filename\n");
		return FALSE;
	}
