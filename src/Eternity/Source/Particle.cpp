...
)
{
	...
	const bool appendGBuffer = gbufferArray != NULL;
	const bool appendShadow =
		shadowArray != NULL &&
		PD->bDeferredMeshParticlesShadows;

	if (!appendGBuffer && !appendShadow)
		return;

	const INT64 baseSortValue =
		static_cast<INT64>(15 * RENDERABLE_USER_SORT_VALUE);
	...
	for (int i = NumAliveParticles - 1; i >= 0; --i)
	{
		...
		if (PE.bDirectionOriented)
		{
			D3DXMATRIX additionalRotation;
			D3DXMatrixRotationYawPitchRoll(
				&additionalRotation,
				R3D_DEG2RAD(PE.vDirOrientedAdditRotation.x),
				R3D_DEG2RAD(PE.vDirOrientedAdditRotation.y - 90),
				R3D_DEG2RAD(PE.vDirOrientedAdditRotation.z)
			);

			r3dVector direction = pt.PrevPosition - pt.Position;

			if ((direction.x * direction.x + direction.y * direction.y + direction.z * direction.z) >= R3D_EPSILON)
			{
				GetRotationMatrixFromVector(direction, &rotation, 0);
			}
			else
			{
		...
		else
		{
			D3DXMatrixRotationYawPitchRoll(
				&rotation,
				R3D_DEG2RAD(pot.AngleY),
				R3D_DEG2RAD(pot.AngleX),
				R3D_DEG2RAD(pot.AngleZ)
			);
		}

		r3dVector meshSize = PE.Mesh->localBBox.Size;

		float meshScaleBase =
			R3D_MAX(
				R3D_MAX(meshSize.x, meshSize.y),
				meshSize.z
			);

		if (meshScaleBase <= R3D_EPSILON)
			meshScaleBase = 1.0f;
		...
		if (appendGBuffer)
		{
			for (int batchIdx = 0; batchIdx < PE.Mesh->NumMatChunks; ++batchIdx)
			{
				const r3dTriBatch& batch = PE.Mesh->MatChunks[batchIdx];
				...
				rend.BatchIdx = batchIdx;
				rend.Color = pot.Color.GetPacked();
				rend.Mesh = PE.Mesh;
				rend.SortValue =
					baseSortValue |
