...
void r3dPerfCounter::Start()
{
  ...
  perf_StartHi = static_cast<long>(ticks >> 32);
  perf_StartLo = static_cast<long>(ticks);
  perf_Start = static_cast<long>(ticks >> 4);
#else
#if !THIS_IN_ECX
  __asm mov ecx, [this]
#endif
...
{
#ifdef _WIN64
  return static_cast<long>((__rdtsc() >> 4) - static_cast<unsigned long>(perf_Start));
#else
#if !THIS_IN_ECX
  __asm mov ecx, [this]
#endif
...
#ifdef _WIN64
  unsigned __int64 ticks = __rdtsc();
  Hi = static_cast<long>(ticks >> 32);
  Lo = static_cast<long>(ticks);
#else
  __asm rdtsc
  __asm mov Hi, edx
  __asm mov Lo, eax
