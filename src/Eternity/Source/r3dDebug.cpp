...
static R3D_FORCEINLINE void SetBits(DWORD_PTR& dw, int lowBit, int bits, int newValue)
{
	DWORD_PTR mask = (static_cast<DWORD_PTR>(1) << bits) - 1; // e.g. 1 becomes 0001, 2 becomes 0011, 3 becomes 0111

	dw = (dw & ~(mask << lowBit)) | (static_cast<DWORD_PTR>(newValue) << lowBit);
}
...
int r3dSetDataBreakpoint( void* address, r3dDataBreakpointByteLen byteLen, int condition )
{
	...
	for( int i = 0; i < (int)g_BreakPointThreads.Count(); i ++ )
	{
		...
		switch ( index )
		{
		case 0: cxt.Dr0 = (DWORD_PTR) address; break;
		case 1: cxt.Dr1 = (DWORD_PTR) address; break;
		case 2: cxt.Dr2 = (DWORD_PTR) address; break;
		case 3: cxt.Dr3 = (DWORD_PTR) address; break;
